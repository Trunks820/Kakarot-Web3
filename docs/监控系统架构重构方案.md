...
> **创建日期**: 2025-11-07  
> **最后更新**: 2025-11-13  
> **状态**: ✅ 核心功能完成（95%）→ ⚡ 性能优化完成 → 🔧 关键问题修复 → 🎉 生产就绪
...
- **配置层（Configuration）**：只描述监控规则（价格涨跌、持仓变化、成交量等），与监控目标无关，可被多个任务复用
- **任务层（Task）**：定义一次具体监控任务（类型/链别/调度策略），绑定**一个配置** ⭐
- **目标层（Target）**：存放要监控的对象（CA/Pair），仅智能监控和批量监控需要；区块监控通常不需要

#### 2. **执行批次管理**

- 把监控目标按批次（≤99个/批）分发给 WebSocket 消费者
- 便于水平扩展、负载均衡和可观测性监控
...
┌─────────────────────────────────────────────────────────────────┐
│  1. 配置层 (Configuration)                                       │
│     monitor_config + monitor_task_config                        │
│     - 存储监控规则配置（价格/持仓/成交量等）                      │
│     - 支持版本控制，可被多个任务复用                             │
│     - 任务与配置是一对多关系（1:N） ⭐ 修正                      │
└─────────────────────────────────────────────────────────────────┘
...
│     monitor_task                                                │
│     - 定义监控任务（类型/链别/调度策略）                          │
│     - 区块监控(block)：全网监听，无需目标                        │
│     - 智能监控(smart)：目标自动生成（市值筛选）                   │
│     - 批量监控(batch)：目标手动输入（用户上传CA）                 │
└─────────────────────────────────────────────────────────────────┘
                           ↓ 关联（可选）
┌─────────────────────────────────────────────────────────────────┐
│  3. 目标层 (Target) - 仅 smart/batch 需要                        │
│     monitor_task_target                                         │
│     - 统一存储监控目标（CA/Pair/Pool）                           │
│     - 支持动态增删，自动重新分配批次                             │
│     - 使用一致性哈希保证批次稳定                                 │
└─────────────────────────────────────────────────────────────────┘
                           ↓ 分配批次
┌─────────────────────────────────────────────────────────────────┐
│  4. 执行层 (Execution)                                           │
│     monitor_batch + monitor_batch_item                          │
│     - 批次头：元信息、心跳、统计                                 │
│     - 批次项：≤99个CA/批，分发给WS消费者                         │
│     - Python监控进程读取并订阅                                   │
└─────────────────────────────────────────────────────────────────┘
                           ↓ 产生告警
┌─────────────────────────────────────────────────────────────────┐
...
#### ✅ **配置与任务一对多** ⭐ 修正
- ~~一个任务可以组合多个配置~~（已废弃）
- **一个任务绑定一个配置**（配置内已包含多个监控规则）
- **一个配置可以被多个任务复用**，修改一次配置，所有任务自动生效
- 说明：单个配置足够一个任务使用（可包含价格+持仓+成交量等多个事件规则）

#### ✅ **目标层统一抽象**
- 智能监控：目标从 `token_launch_history` 自动筛选，定期同步
- 批量监控：用户手动输入CA列表，一次性导入
- 区块监控：无需目标层，直接全网监听

#### ✅ **批次分配算法**
- 使用一致性哈希对目标排序，保证多次分配时大部分批次不变
- 仅对新增/删除的目标调整批次，减少WS连接的重建开销
...
#### 1. **monitor_config** (监控配置表) - 可复用的规则定义

```sql
CREATE TABLE monitor_config (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '配置ID',
  config_name VARCHAR(100) NOT NULL COMMENT '配置名称',
  config_type VARCHAR(20) DEFAULT 'composite' COMMENT '配置类型：price/holder/volume/composite',
  chain_type VARCHAR(10) NOT NULL COMMENT '链类型：sol/bsc/eth',
  
  -- 🆕 重要字段（与现有 global_monitor_config 对齐）
  source VARCHAR(20) COMMENT '数据源筛选：all/pump/fourmeme',
  market_type VARCHAR(20) DEFAULT 'external' COMMENT '市场类型：internal-内盘/external-外盘',
  min_transaction_usd DECIMAL(20,2) COMMENT '单笔最小交易金额(USD)',
  cumulative_min_amount_usd DECIMAL(20,2) COMMENT '累计最小金额(USD)',
  
  -- 监控配置（JSON格式，灵活扩展）
  time_interval VARCHAR(10) DEFAULT '5m' COMMENT '时间周期：1m/5m/1h/24h',
  top_holders_threshold DECIMAL(5,2) COMMENT '前十持仓阈值%',
  events_config JSON COMMENT '事件配置: {priceChange:{enabled,risePercent,fallPercent}, holders:{...}, volume:{...}}',
  trigger_logic VARCHAR(10) DEFAULT 'any' COMMENT '触发逻辑：any-任一满足/all-全部满足',
  notify_methods VARCHAR(100) DEFAULT 'telegram' COMMENT '通知方式：telegram,wechat,email',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='监控配置表-可复用的规则定义';

-- 示例数据
INSERT INTO monitor_config (config_name, config_type, chain_type, events_config, trigger_logic) VALUES
('BSC 价格暴涨预警', 'price', 'bsc', '{"priceChange":{"enabled":true,"risePercent":30}}', 'any'),
('SOL 持仓集中度预警', 'holder', 'sol', '{"holders":{"enabled":true,"increasePercent":20}}', 'any'),
('综合监控-高风险', 'composite', 'bsc', '{"priceChange":{"enabled":true,"risePercent":50},"volume":{"enabled":true,"threshold":10000}}', 'all');
```

#### 2. **monitor_task** (监控任务表) - 定义具体监控

```sql
CREATE TABLE monitor_task (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '任务ID',
  task_name VARCHAR(100) NOT NULL COMMENT '任务名称',
  task_type VARCHAR(20) NOT NULL COMMENT '任务类型：smart-智能/batch-批量/block-区块',
  chain_type VARCHAR(10) NOT NULL COMMENT '链类型：sol/bsc/eth',
  ...
  auto_sync_targets TINYINT DEFAULT 1 COMMENT '是否自动同步目标(仅smart)',
  sync_interval_minutes INT DEFAULT 30 COMMENT '同步间隔分钟数(仅smart)',
  
  -- 批量监控专用字段
  target_count INT DEFAULT 0 COMMENT '目标数量(仅batch)',
  
  -- 调度配置
  schedule_cron VARCHAR(50) COMMENT '定时规则(可选)',
  
  -- 执行状态
  status TINYINT DEFAULT 1 COMMENT '状态：1-启用/0-停用/2-暂停',
...
#### 3. **monitor_task_config** (任务配置关联表) - ~~多对多映射~~ 已简化为直接关联 ⭐
...
CREATE TABLE monitor_task_config (
  ...
  config_order INT DEFAULT 0 COMMENT '配置优先级(数字越小越优先)',
  override_settings JSON COMMENT '覆盖配置(可选)',
  is_active TINYINT DEFAULT 1 COMMENT '是否启用此配置',
  create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='任务配置关联表';
```

#### 4. **monitor_task_target** (监控目标表) - 统一存储目标

```sql
CREATE TABLE monitor_task_target (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '目标ID',
  task_id BIGINT NOT NULL COMMENT '任务ID',
  target_type VARCHAR(20) NOT NULL COMMENT '目标类型：ca/pair/pool',
  target_value VARCHAR(255) NOT NULL COMMENT '目标值(合约地址/交易对等)',
  ...
  source VARCHAR(20) COMMENT '来源：pump/fourmeme/manual',
  twitter_url VARCHAR(255) COMMENT 'Twitter链接',
  
  -- 批次分配
  batch_id INT COMMENT '分配的批次ID',
  batch_order INT COMMENT '批次内序号(0-98)',
  
  -- 状态管理
  status TINYINT DEFAULT 1 COMMENT '状态：1-活跃/0-停用/2-已删除',
...
#### 5. **monitor_batch** (批次头表) - 批次元信息

```sql
CREATE TABLE monitor_batch (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '批次ID',
  task_id BIGINT NOT NULL COMMENT '任务ID',
  batch_no INT NOT NULL COMMENT '批次编号(从1开始)',
  item_count INT DEFAULT 0 COMMENT '本批次项数(≤99)',
  
  -- 执行状态
  status VARCHAR(20) DEFAULT 'pending' COMMENT '状态：pending/running/paused/stopped',
  worker_id VARCHAR(50) COMMENT '执行节点ID',
  worker_pid INT COMMENT '进程ID',
  last_heartbeat DATETIME COMMENT '最后心跳时间',
  
  -- 统计信息
  total_alerts INT DEFAULT 0 COMMENT '累计告警数',
  ...
  FOREIGN KEY (task_id) REFERENCES monitor_task(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='批次头表';
```

#### 6. **monitor_batch_item** (批次项表) - 批次详情
...
CREATE TABLE monitor_batch_item (
  ...
  item_order INT COMMENT '批次内序号(0-98)',
  
  -- 冗余字段(避免JOIN)
  ca VARCHAR(255) COMMENT 'Token合约地址',
  token_name VARCHAR(100) COMMENT 'Token名称',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='批次项表';
```

#### 7. **monitor_alert_log** (监控告警日志表) - 统一告警

```sql
CREATE TABLE monitor_alert_log (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '告警ID',
  task_id BIGINT NOT NULL COMMENT '关联任务ID',
  config_id BIGINT COMMENT '触发的配置ID',
  
  -- 去重key(MD5: task_id+ca+alert_type+date)
  alert_key VARCHAR(32) COMMENT '告警去重键',
  ...
  -- Token信息(可选)
  ca VARCHAR(255) COMMENT 'Token地址',
  token_name VARCHAR(100) COMMENT 'Token名称',
  token_symbol VARCHAR(50) COMMENT 'Token符号',
  market_cap DECIMAL(20,2) COMMENT '市值',
  
  -- 告警详情
  alert_type VARCHAR(50) NOT NULL COMMENT '告警类型：price_change/holder_change/volume_change/block_event',
  alert_title VARCHAR(200) COMMENT '告警标题',
  alert_content JSON COMMENT '告警内容(详细数据)',
  trigger_value DECIMAL(20,4) COMMENT '触发值',
  
  -- 通知状态
  notify_status VARCHAR(20) DEFAULT 'pending' COMMENT '通知状态：pending/sent/failed/skipped',
  notify_channels VARCHAR(100) COMMENT '通知渠道：telegram,wechat',
  notify_time DATETIME COMMENT '通知时间',
  notify_error TEXT COMMENT '通知错误信息',
  
  -- 性能指标
  detect_latency_ms INT COMMENT '检测延迟(毫秒)',
  notify_latency_ms INT COMMENT '通知延迟(毫秒)',
...
def execute_block_monitor(task_id):
    ...
    configs = load_task_configs(task_id)  # 从 monitor_task_config 加载
...
### 3. 智能监控（Smart Monitor）- 自动生成

**特点**：从 token_launch_history 自动筛选，幂等增量对账
...
def sync_smart_monitor_targets(task_id):
    ...
    # 3. 幂等插入（ON DUPLICATE KEY UPDATE）
...
class BatchAllocator:
    ...
    def allocate_batches(self, task_id):
        ...
        if updates:
            ...
            log.info(f"任务 {task_id}: {len(targets_sorted)} 个目标, {total_batches} 个批次, 变更 {len(updates)} 项")
...
| 方案 | 实时性 | 复杂度 | 可靠性 | 推荐 |
|------|--------|--------|--------|------|
| A. 轮询增量 | 5-15s | 低 | 高 | ✅ 兜底必须 |
| B. Redis Pub/Sub | <1s | 中 | 中 | ✅ 推荐 |
| C. CDC/Binlog | <0.5s | 高 | 高 | 大规模 |
...
public class MonitorConfigServiceImpl {
    ...
    public int updateMonitorConfig(MonitorConfig config) {
        ...
        if (result > 0) {
            ...
            log.info("📢 配置变更事件: configId={}, version={}", config.getId(), config.getVersion());
        }
...
class ConfigWatcher:
    ...
    def _reload_config(self, config_id, version):
        ...
        if config:
            ...
            log.info(f"✅ 配置已重载: configId={config_id}, version={version}")
```
...
| 场景 | 数据量 | 并发数 | 预期响应时间 | 压测工具 |
|------|--------|--------|--------------|----------|
| 查询活跃任务列表 | 2000条 | 100 QPS | < 50ms | JMeter |
| 查询单个任务配置 | - | 500 QPS | < 10ms | ab |
| 创建批量监控任务 | 99个CA | 10 QPS | < 200ms | JMeter |
| 批量更新执行状态 | 99条/批 | 50 QPS | < 100ms | ab |
| 告警日志写入 | - | 200 QPS | < 50ms | JMeter |
...
| 指标 | 旧架构 | 新架构 | 提升 |
|------|--------|--------|------|
| 配置查询 | ~200ms (JOIN+GROUP BY) | ~10ms (直接查询) | **95%↑** |
| 批次列表 | ~500ms (复杂GROUP BY) | ~50ms (单表查询) | **90%↑** |
| 告警写入 | ~80ms | ~30ms | **62.5%↑** |
| 数据冗余率 | 98% | 2% | **96%↓** |
...
**问题**: `monitor_task_target` 目前没有 `chain_type`（靠任务继承），推荐冗余一份便于跨任务运维/排障与独立查询。

**解决方案**:

```sql
-- 地址规范化：
-- EVM 用 LOWER() 存储，列型 CHAR(42)
-- Solana base58 长度不定，列型 VARCHAR(64) 足够

ALTER TABLE monitor_task_target 
  ADD COLUMN chain_type VARCHAR(10) NULL COMMENT '链类型（冗余便于查询）' AFTER task_id,
...
```sql
ALTER TABLE monitor_alert_log 
  ADD COLUMN config_version INT NULL COMMENT '触发时的配置版本' AFTER config_id;
```

#### 3. 批次唯一性与反查优化

**问题**: 现在批次项能反查目标，但目标反查批次头要多跳。

**解决方案**:

```sql
ALTER TABLE monitor_batch_item 
  ADD COLUMN task_id BIGINT NULL COMMENT '任务ID（冗余加速反查）',
  ADD COLUMN batch_no INT NULL COMMENT '批次号（冗余加速反查）',
...
#### 4. JSON 可检索字段生成列（MySQL 8.0+）

**问题**: 常用阈值做生成列 + 索引，提升查询性能。
...
**问题**: 目前去重键是 `task_id+ca+alert_type+date`，需加 `config_id|config_version` 更精准；同时 block 类可能无 ca。

**解决方案**:

```sql
ALTER TABLE monitor_alert_log 
  ADD COLUMN event_key VARCHAR(64) NULL COMMENT '区块类事件唯一键（无ca时使用）';
...
**方案**: 按 `hash(task_id + target_value + salt_v1)` 生成，未来若迁移算法，变更 `salt_v2` 只影响新目标，老批次稳定。
...
# 迁移时：新目标用 v2，老目标保持 v1
```

#### 2. 硬约束 ≤99/批（数据库保护）

**方案**: DB 侧无法直接 CHECK，可在写入批次项时做保护性断言（事务内计数 + 失败回滚）。
...
def add_target_to_batch(conn, batch_id, target_id, item_order):
    ...
    with conn.cursor() as cursor:
        ...
        if current_count >= 99:
            raise ValueError(f"批次 {batch_id} 已满（{current_count}/99）")
...
**方案**: 心跳超时自动 `status=paused`，并发起孤儿批次回收（幂等重绑到空闲 worker）。

```python
def orphan_batch_recovery():
    """孤儿批次回收（定时任务，每分钟执行）"""
    HEARTBEAT_TIMEOUT = 300  # 5分钟无心跳视为孤儿
    ...
    for batch in orphan_batches:
        logger.warning(f"🚨 发现孤儿批次: {batch['id']}, worker={batch['worker_id']}")
        
        # 暂停批次
        db.execute("""
            UPDATE monitor_batch
            SET status = 'paused',
                error_message = CONCAT('心跳超时，上次心跳: ', last_heartbeat)
            WHERE id = %s
        """, (batch['id'],))
        
        # 发送告警
        send_alert(f"批次 {batch['id']} 心跳超时，已自动暂停")
```
...
class ConfigChangeDebouncer:
    ...
    def trigger(self, task_id: int):
        ...
        with self.lock:
            ...
            logger.debug(f"⏳ 任务 {task_id} 配置变更，{self.delay}秒后重载")
...
**方案**: 任务编辑多表写时，将 `monitor_task.status='updating'`，全部写完后 `enabled`，监控端见到 `updating` 直接跳过。
...
def update_task_with_configs(task_id: int, task_data: dict, configs: list):
    ...
    logger.info(f"✅ 任务 {task_id} 配置原子更新完成")
```
...
def load_task_configs(task_id: int):
    ...
    if task['status'] == 'updating':
        logger.debug(f"⏸️ 任务 {task_id} 正在更新中，跳过本轮")
        return None
...
**问题**: 文档中迁移仍引用了 `ws_batch_execution_new`（v1 遗留），应改为 v2.0 的目标 → 分批 → 批次头/项三步。

**正确的迁移步骤**:

```sql
-- ============================================================
-- 步骤 2.3: 迁移 Token 到目标层
-- ============================================================
INSERT INTO monitor_task_target (
...
-- ============================================================
-- 步骤 2.4: 批次分配（离线批处理脚本执行一次）
-- ============================================================
-- 使用一致性哈希方法计算 batch_no & item_order
-- 回写 monitor_task_target.batch_id, batch_order
...
def allocate_batches_for_all_tasks():
    ...
    for task in tasks:
        ...
        logger.info(f"✅ 任务 {task_id} 分配了 {len(batch_assignments)} 个批次")
...
-- 校验目标数量
SELECT '目标层数量' AS stage, COUNT(*) AS count FROM monitor_task_target
UNION ALL
SELECT '批次项数量' AS stage, COUNT(*) AS count FROM monitor_batch_item;
...
**方案**: `monitor_alert_log` 建议**月分区 + task_id 子分区**以均衡写入热点。
...
**方案**: 对同 CA 在 5 分钟内的相同 `alert_type` 做聚合窗口（计数/最大触发值/首次时间），降低刷屏。
...
class AlertAggregator:
    ...
    def add_alert(self, ca: str, alert_type: str, trigger_value: float) -> bool:
        ...
        with self.lock:
            ...
            # 窗口内后续触发 -> 不发送（但记录）
            logger.debug(f"📊 告警聚合: {key}, 窗口内累计 {len(self.cache[key])} 次")
            return False
...
def process_alert(ca, alert_type, trigger_value):
    ...
    if should_send:
        ...
        logger.info(f"📉 告警已聚合: {ca} {alert_type}, 窗口内{summary['count']}次")
```
...
def add_target(task_id, chain_type, ca):
    if not validate_address(chain_type, ca):
        raise ValueError(f"无效的 {chain_type} 地址: {ca}")
...
**方案**: 批量/智能物化采用 `INSERT … ON DUPLICATE KEY UPDATE`，并记录 `first_seen_time`（便于做"新生 token"策略）。

```sql
-- 修改表结构
ALTER TABLE monitor_task_target
ADD COLUMN first_seen_time DATETIME NULL COMMENT '首次发现时间',
ADD COLUMN last_seen_time DATETIME NULL COMMENT '最后发现时间',
...
# 新生 token 策略（发现时间 < 1小时）
def get_new_tokens(task_id: int, hours: int = 1):
    """获取新生 token（首次发现时间在N小时内）"""
    return db.fetchall("""
...
**方案**: 在执行器前全局限流（Redis 令牌桶），不同链/供应商（NodeReal/Chainstack）单独桶，防止突发超限。
...
class RateLimiter:
    ...
    def acquire(self, key: str, rate: int, capacity: int) -> bool:
        """
        尝试获取令牌
        :param key: 限流键（如 'rpc:bsc:nodereal'）
        :param rate: 每秒填充速率
        :param capacity: 桶容量
        :return: 是否成功获取
        """
...
def call_rpc(chain: str, provider: str, method: str, params: list):
    ...
    if not limiter.acquire(key, limits['rate'], limits['capacity']):
        logger.warning(f"🚦 RPC限流触发: {key}")
        time.sleep(0.1)  # 短暂等待
        if not limiter.acquire(key, limits['rate'], limits['capacity']):
            raise Exception(f"RPC限流超限: {key}")
...
4. [MySQL 8.0 分区表最佳实践](https://dev.mysql.com/doc/refman/8.0/en/partitioning.html)
5. [Redis 限流算法](https://redis.io/docs/reference/patterns/rate-limiting/)
6. [一致性哈希原理](https://en.wikipedia.org/wiki/Consistent_hashing)
...
```
监控V2 (crypto:monitor-v2)
├── 📊 监控总览 (crypto:monitor-v2:view)
│   路由: /monitor-v2/dashboard
│   组件: views/monitor-v2/dashboard/index.vue
│   说明: 首页看板，包含3个快捷卡片 + 可视化图表
│
├── ⚙️ 配置管理 (crypto:monitor-v2:config)
│   路由: /monitor-v2/config
│   组件: views/monitor-v2/config/index.vue
│   说明: 完整的配置管理页面（列表、新增、编辑、详情）
│   权限按钮:
│     - 查询 (crypto:monitor-v2:config:list)
│     - 新增 (crypto:monitor-v2:config:add)
│     - 修改 (crypto:monitor-v2:config:edit)
│     - 删除 (crypto:monitor-v2:config:remove)
│     - 详情 (crypto:monitor-v2:config:query)
│
├── 📝 任务管理 (crypto:monitor-v2:task)
│   路由: /monitor-v2/task
│   组件: views/monitor-v2/task/index.vue
│   说明: 完整的任务管理页面（3种类型任务）
│   权限按钮:
│     - 查询 (crypto:monitor-v2:task:list)
│     - 新增 (crypto:monitor-v2:task:add)
│     - 修改 (crypto:monitor-v2:task:edit)
│     - 删除 (crypto:monitor-v2:task:remove)
│     - 详情 (crypto:monitor-v2:task:query)
│     - 启动 (crypto:monitor-v2:task:start)
│     - 停止 (crypto:monitor-v2:task:stop)
│
├── ⚡ 批次管理 (crypto:monitor-v2:batch)
│   路由: /monitor-v2/batch
│   组件: views/monitor-v2/batch/index.vue
│   说明: 完整的批次管理页面（列表、详情、监控）
│   权限按钮:
│     - 查询 (crypto:monitor-v2:batch:list)
│     - 详情 (crypto:monitor-v2:batch:query)
│     - 重启 (crypto:monitor-v2:batch:restart)
│
└── 🔔 告警日志 (crypto:monitor-v2:alert)
    路由: /monitor-v2/alert
    组件: views/monitor-v2/alert/index.vue
    说明: 完整的告警管理页面（列表、处理、统计）
    权限按钮:
      - 查询 (crypto:monitor-v2:alert:list)
      - 详情 (crypto:monitor-v2:alert:query)
      - 处理 (crypto:monitor-v2:alert:handle)
```
...
**文件路径**: `views/monitor-v2/dashboard/index.vue`  
**当前状态**: ✅ 已完成（对应现有的 `views/index.vue`）
...
│  📊 可视化图表区（待开发）                                   │
│  ┌────────────────────────────────────────────────────────┐│
│  │  配置-任务-批次关系图（Sankey 桑基图）                  ││
│  │                                                          ││
│  │  配置A ──┐                                               ││
│  │         ├──→ 任务1 ──→ 批次001（运行中，10项）         ││
│  │  配置B ──┤                                               ││
│  │         └──→ 任务2 ──→ 批次002（暂停，15项）           ││
│  │  配置C ──────→ 任务3 ──→ 批次003（心跳超时，8项）      ││
│  │                                                          ││
│  └────────────────────────────────────────────────────────┘│
│                                                              │
│  ┌───────────────────┐  ┌───────────────────┐              │
...
│  ┌────────────────────────────────────────────────────────┐│
│  │ 最近告警（Top 10）                                      ││
│  │ - Token XXX 价格上涨 25%  [5分钟前]   [查看详情]      ││
│  │ - Token YYY 心跳超时      [10分钟前]  [查看详情]      ││
│  │ ...                                                     ││
│  └────────────────────────────────────────────────────────┘│
│                                                              │
│  ┌──────────────────┐  ┌──────────────────┐                │
...
**核心功能:**
- ✅ 3个快捷卡片（ConfigCard、TaskCard、BatchCard）
- ⏳ 配置-任务-批次关系可视化（Sankey图）
- ⏳ 批次状态分布图（饼图）
- ⏳ 任务类型分布图（柱状图）
- ⏳ 最近告警列表（Top 10）
- ⏳ 任务变更时间线
...
**文件路径**: `views/monitor-v2/config/index.vue`  
**当前状态**: ⏳ 待开发

**页面功能:**
1. **配置列表**
   - 表格展示（分页、搜索、筛选）
   - 字段：配置名称、类型、链类型、状态、关联任务数、创建时间
   - 筛选条件：配置类型、链类型、状态（启用/停用）
   
2. **配置新增**
   - 表单对话框/抽屉
   - 支持价格、持仓、交易量等监控规则
   - 动态配置 `eventsConfig` 字段
...
**文件路径**: `views/monitor-v2/task/index.vue`  
**当前状态**: ⏳ 待开发

**页面功能:**
1. **任务列表**
   - 表格展示（分页、搜索、筛选）
   - 字段：任务名称、类型、配置、状态、运行批次数、创建时间
   - 筛选条件：任务类型、链类型、状态（运行/停止）

2. **任务新增**
   - 支持3种任务类型：
     - 智能监控（市值范围、Twitter要求、自动同步）
     - 批量监控（手动输入CA列表）
     - 区块监控（全链监听）
   - 关联配置（单选）
...
**文件路径**: `views/monitor-v2/batch/index.vue`  
**当前状态**: ⏳ 待开发

**页面功能:**
1. **批次列表**
   - 表格展示（分页、搜索、筛选）
   - 字段：批次号、任务、Worker、状态、项数、告警数、错误数、心跳时间
   - 筛选条件：任务、状态、Worker
...
**文件路径**: `views/monitor-v2/alert/index.vue`  
**当前状态**: ⏳ 待开发（Phase 3）

**页面功能:**
1. **告警列表**
   - 表格展示（分页、搜索、筛选）
   - 字段：Token、告警类型、触发条件、时间、状态、处理人
   - 筛选条件：告警类型、状态、时间范围
...
// API: GET /crypto/monitor-v2/dashboard/relation
{
  ...
  links: [
    { source: '配置A：价格监控', target: '任务1：智能监控', value: 1 },
    { source: '任务1：智能监控', target: '批次001', value: 10 }
  ]
}
```
...
```javascript
{
  series: [{
    ...
    radius: ['40%', '70%'],
    data: [
      { value: 10, name: '运行中', itemStyle: { color: '#67C23A' } },
      { value: 5, name: '暂停', itemStyle: { color: '#E6A23C' } },
      { value: 2, name: '心跳超时', itemStyle: { color: '#F56C6C' } }
    ]
  }]
}
```
...
```javascript
{
  xAxis: { type: 'category', data: ['智能监控', '批量监控', '区块监控'] },
  yAxis: { type: 'value' },
  series: [{
    type: 'bar',
    data: [12, 8, 5],
...
> **当前进度**: 80% ✅  
> **状态**: 架构完成、前端首页完成、后端API完成、批次管理API已完成  
> **最后更新**: 2025-11-10
...
已创建所有7张核心表，字段根据实际需求调整：

| 表名 | 说明 | 实际表名 | 状态 |
|------|------|----------|------|
| 配置表 | 监控配置（含版本控制） | `monitor_config_v2` | ✅ |
| 任务表 | 监控任务（smart/batch/block） | `monitor_task_v2` | ✅ |
| 任务配置关联表 | ~~M:N关联~~ 1:1关联 ⭐ | `monitor_task_config_v2` | ✅ |
| 任务目标表 | 监控目标（CA/Pair） | `monitor_task_target_v2` | ✅ |
| 批次头表 | 批次执行状态 | `monitor_batch_v2` | ✅ |
| 批次项表 | 批次明细（≤99项） | `monitor_batch_item_v2` | ✅ |
| 告警日志表 | 告警记录（去重） | `monitor_alert_log_v2` | ✅ |

**关键调整：**
- ✅ `monitor_config_v2`: 字段名 `config_type` → `config_category`，新增 `source` 字段
- ✅ `monitor_task_v2`: 移除区块监控专用字段（`market_type`/`minTransactionUsd`等），由配置层负责
- ✅ 所有表名统一使用 `_v2` 后缀，避免与旧系统冲突
...
|------|-----------|---------|--------|------|
| **配置管理** | `MonitorConfigController` | `MonitorConfigServiceImpl` | `MonitorConfigMapper.xml` | ✅ |
| **任务管理** | `MonitorTaskController` | `MonitorTaskServiceImpl` | `MonitorTaskMapper.xml` | ✅ |
| **任务配置关联** | - | - | `MonitorTaskConfigMapper.xml` | ✅ |
| **任务目标管理** | - | - | `MonitorTaskTargetMapper.xml` | ✅ |

##### API端点清单

**配置管理 (`/crypto/monitor-v2/config`)**
```java
GET    /list                    // 查询配置列表
GET    /{id}                    // 获取配置详情 @PreAuthorize
POST   /                        // 新增配置
PUT    /                        // 修改配置 @PreAuthorize
DELETE /{ids}                   // 删除配置 @PreAuthorize
PUT    /{id}/status             // 启用/停用配置
GET    /stats                   // 配置统计
```
...
```java
GET    /list                    // 查询任务列表
GET    /{id}                    // 获取任务详情 @PreAuthorize
POST   /smart                   // 新增智能监控任务
POST   /batch                   // 新增批量监控任务
POST   /block                   // 新增区块监控任务
PUT    /                        // 修改任务 @PreAuthorize
DELETE /{ids}                   // 删除任务 @PreAuthorize
PUT    /{id}/start              // 启动任务
PUT    /{id}/stop               // 停止任务
GET    /stats                   // 任务统计
```
...
**权限注解已添加：**
- ✅ `@PreAuthorize("@ss.hasPermi('crypto:monitor-v2:config:query')")` - 配置详情/编辑/删除
- ✅ `@PreAuthorize("@ss.hasPermi('crypto:monitor-v2:task:query')")` - 任务详情/编辑/删除
...
| 组件 | 文件 | 功能 | 状态 |
|------|------|------|------|
| **主页** | `views/index.vue` | 监控系统 V2.0 首页 | ✅ |
| **配置卡片** | `components/monitor/ConfigCard.vue` | 配置管理（新建/编辑/列表） | ✅ |
| **任务卡片** | `components/monitor/TaskCard.vue` | 任务管理（新建/详情/列表） | ✅ |
| **批次卡片** | `components/monitor/BatchCard.vue` | 批次状态监控 | ✅ |

##### 核心功能实现

**ConfigCard（配置卡片）：**
- ✅ 弹窗式新建/编辑配置（无需页面跳转）
- ✅ 支持价格、持仓、交易量监控规则（基于原卡片设计）
- ✅ 动态显示 `eventsConfig` 字段
- ✅ 配置列表管理（查看/编辑/删除/启停）
- ✅ 调用后端API（真实数据，已移除Mock）

**TaskCard（任务卡片）：**
- ✅ 三种任务类型：智能监控/批量监控/区块监控
- ✅ 区块监控去除冗余字段（内外盘/金额阈值由配置决定）
- ✅ 关联配置改为**单选**（所有任务类型统一）
- ✅ 任务列表管理（查看详情/启停/删除）
- ✅ 调用后端API（真实数据，已移除Mock）

**首页统计（index.vue）：**
- ✅ 调用 `getConfigStats()` - 配置统计
- ✅ 调用 `getTaskStats()` - 任务统计
- ✅ 调用 `getBatchStats()` - 批次统计
- ✅ 移除所有Mock数据

##### 前端API集成

**文件：** `src/api/crypto/monitor-v2.js`
...
| 问题 | 原因 | 解决方案 | 状态 |
|------|------|----------|------|
| Spring Bean注入冲突 | `TokenMonitorConfigServiceImpl` 中变量名冲突 | 重命名 `monitorConfigMapper` → `tokenMonitorConfigMapper` | ✅ |
| 表不存在错误 | 表名未加 `_v2` 后缀 | 所有Mapper XML更新表名 | ✅ |
| 字段不存在 | 字段名不匹配 | `config_type` → `config_category`，移除虚拟列 | ✅ |
| MyBatis参数绑定异常 | 多参数方法未标注 `@Param` | 添加 `@Param` 注解 | ✅ |
| 区块监控字段冗余 | 任务层保存配置层字段 | 移除任务层区块监控字段，统一由配置决定 | ✅ |
| 配置选择逻辑混乱 | 单选/多选不统一 | 统一改为单选 `configId` | ✅ |
...
│   ├── 查询（crypto:monitor-v2:config:list）
│   ├── 新增（crypto:monitor-v2:config:add）
│   ├── 编辑（crypto:monitor-v2:config:edit）
│   ├── 删除（crypto:monitor-v2:config:remove）
│   └── 详情（crypto:monitor-v2:config:query）
├── 任务管理
│   ├── 查询（crypto:monitor-v2:task:list）
│   ├── 新增（crypto:monitor-v2:task:add）
│   ├── 编辑（crypto:monitor-v2:task:edit）
│   ├── 删除（crypto:monitor-v2:task:remove）
│   └── 详情（crypto:monitor-v2:task:query）
└── 批次管理
    ├── 查询（crypto:monitor-v2:batch:list）
    ├── 详情（crypto:monitor-v2:batch:query）
    ├── 重启（crypto:monitor-v2:batch:restart）
    └── 导出（crypto:monitor-v2:batch:export）
```

#### 2. **批次管理功能（✅ 已完成 - 2025-11-10）**

- ✅ 批次统计接口（`/stats`）
- ✅ 批次列表查询（`/list`）
- ✅ 批次详情查询（`/{id}`）
- ✅ 批次项明细查询（`/{batchId}/items`）
- ✅ 批次重启功能（`/{id}/restart`）
- ✅ 批次心跳监控（`/{id}/heartbeat`）
- ✅ 批次心跳更新（`POST /{id}/heartbeat`）

**创建的文件**:
- `MonitorBatchController.java` - 控制器
- `IMonitorBatchService.java` - 服务接口
- `MonitorBatchServiceImpl.java` - 服务实现
- `MonitorBatch.java` - 批次实体
- `MonitorBatchItem.java` - 批次项实体
- `MonitorBatchMapper.java` - Mapper接口
- `MonitorBatchMapper.xml` - SQL配置

#### 3. **告警日志功能（✅ 已完成 - 2025-11-11）**
...
**创建的文件**:
- `MonitorAlertLogController.java` - 控制器
- `IMonitorAlertLogService.java` - 服务接口
- `MonitorAlertLogServiceImpl.java` - 服务实现
- `MonitorAlertLog.java` - 告警实体
- `MonitorAlertLogMapper.java` - Mapper接口
- `MonitorAlertLogMapper.xml` - SQL配置
- `alert/index.vue` - 前端页面

#### 4. **WebSocket实时推送（✅ 已完成 - 2025-11-10）**
...
**创建的文件**:
- `MonitorWebSocketHandler.java` - WebSocket处理器
- `MonitorWebSocketConfig.java` - WebSocket配置
...
#### Phase 3: 告警系统（✅ 已完成 - 2025-11-11）
1. ✅ 实现告警日志记录
2. ✅ 实现告警去重逻辑（alert_key）
3. ✅ 实现告警统计查询
4. ✅ 告警中心页面开发

#### Phase 4: 实时通知（✅ 已完成 - 2025-11-10）
1. ✅ WebSocket服务端实现
2. ✅ 前端WebSocket连接
3. ✅ 实时推送功能测试

#### Phase 5: 零停机迁移（✅ 跳过 - 2025-11-11）
1. ✅ 决定不迁移旧数据，直接使用新系统
2. ✅ 旧系统保留但不再维护
3. ✅ 新系统独立运行

#### Phase 6: 性能优化（✅ 已完成 - 2025-11-11）
1. ✅ 批量插入优化（target表 + batch_item表）
2. ✅ 批量大小：500条/批
3. ✅ 智能筛选条件（市值、Twitter）
4. ✅ 性能提升：30秒 → 1-2秒（50倍）
...
### 议题：任务与配置的关联方式（单选 vs 多选）

> **提出时间**: 2025-11-09  
> **紧急程度**: ⭐⭐⭐ 高（影响数据模型和业务逻辑）  
> **需要决策**: 是否允许一个任务关联多个配置
> **✅ 已决策**: 2025-11-13 - 采用方案1（一个任务一个配置），理由：单个配置足够一个任务使用
...
配置示例：
{
  "configName": "SOL外盘全方位监控",
  "chainType": "sol",
  "marketType": "external",
  "minTransactionUsd": 500,
  "eventsConfig": [
    {"eventType": "price", "priceRisePct": 20},      // 事件1：价格监控
    {"eventType": "volume", "volThresholdUsd": 10000}, // 事件2：交易量监控
    {"eventType": "holder", "holderConcentrationPct": 60} // 事件3：持仓监控
  ],
  "triggerLogic": "any"
}
```
...
**优点：**
- ✅ **简单直观** - 1个任务对应1套完整规则
- ✅ **配置即规则集** - `eventsConfig` 可包含多个事件
- ✅ **避免冲突** - 不存在多配置触发去重问题
- ✅ **职责清晰** - 任务定义"监控什么"，配置定义"如何监控"
- ✅ **复用性好** - 多个任务可使用同一配置

**使用场景：**
```
场景1: 不同市值范围，相同监控规则
任务A [市值10万-100万]   → 配置1 [价格+交易量+持仓监控]  ✅ 配置复用
任务B [市值100万-1000万] → 配置1 [价格+交易量+持仓监控]
任务C [市值>1000万]      → 配置1 [价格+交易量+持仓监控]

场景2: 区块监控，内外盘分开
任务A [SOL链区块监控]   → 配置1 [外盘 + 价格/交易量事件]
任务B [SOL链区块监控]   → 配置2 [内盘 + 持仓/流动性事件]
```

**缺点：**
- ❌ **配置冗余风险** - 如果需要不同事件组合，需创建多个配置
- ❌ **灵活性略低** - 无法动态组合不同配置的规则
...
**缺点：**
- ❌ **复杂度高** - 需要处理多配置触发逻辑
- ❌ **去重困难** - 同一Token触发多个配置时的通知去重
- ❌ **理解成本** - 用户需理解配置组合的效果
- ❌ **调试困难** - 告警来源不明确

**去重挑战：**
```
任务A + 配置1（价格监控） → Token X涨25% → 触发告警
任务A + 配置2（交易量监控） → Token X交易量激增 → 触发告警
...
┌──────────────────────────────────────────────────────┐
│                   任务层 (Task)                       │
│  负责：监控"什么" (What to Monitor)                    │
├──────────────────────────────────────────────────────┤
│ 智能监控：市值范围、Twitter要求、自动同步逻辑          │
│ 批量监控：指定的目标列表（手动输入CA）                 │
│ 区块监控：全链监听（无目标筛选）                        │
└──────────────────────────────────────────────────────┘
                          ↓ M:1 关联
┌──────────────────────────────────────────────────────┐
│                  配置层 (Config)                      │
│  负责：监控"如何" (How to Monitor)                     │
├──────────────────────────────────────────────────────┤
│ • 监控规则：价格变化、交易量、持仓集中度                │
│ • 基础参数：market_type、金额阈值、时间间隔            │
│ • 触发逻辑：any/all                                   │
│ • 通知方式：telegram/email/webhook                    │
│ • 事件集合：eventsConfig JSON数组                     │
└──────────────────────────────────────────────────────┘
```
...
```
任务层保存的字段：
  ✅ minMarketCap / maxMarketCap (智能监控)
  ✅ hasTwitter (智能监控)
  ✅ autoSyncTargets / syncIntervalMinutes (智能监控行为)
  ❌ marketType / minTransactionUsd (已删除，移至配置层)

配置层保存的字段：
  ✅ market_type (内外盘)
  ✅ minTransactionUsd / cumulativeMinAmountUsd (区块监控阈值)
  ✅ eventsConfig (事件规则：价格、交易量、持仓)
  ✅ triggerLogic / notifyMethods
```

**为什么这样设计合理？**
1. **智能监控的市值筛选** → 任务层
   - 原因：市值是**目标筛选条件**，决定"监控哪些Token"
   - 不同任务的市值范围不同，但可使用相同监控规则

2. **区块监控的内外盘/金额阈值** → 配置层
   - 原因：这是**监控规则的一部分**，决定"如何过滤交易"
   - 区块监控没有目标列表，所有筛选都是规则
...
**理由：**
1. ✅ **配置功能已足够强大** - `eventsConfig` 可包含多个事件规则
2. ✅ **避免去重复杂度** - 单配置不存在多触发源问题
3. ✅ **符合当前架构** - 职责分离清晰（任务定义目标，配置定义规则）
4. ✅ **用户体验好** - 简单直观，配置即完整规则集
5. ✅ **可扩展性好** - 未来可在配置内增加更多事件类型
...
- **方案1**: 在配置层增加更多事件类型（推荐）
  ```json
  {
    "eventsConfig": [
      ...
      {"eventType": "volume", ...},
      {"eventType": "holder", ...},
      {"eventType": "liquidity", ...},  // 新增
      {"eventType": "whale_tracking", ...}  // 新增
    ]
  }
  ```
...
### 监控总览页面性能优化 (2025-11-10)

#### 问题描述
监控总览页面（`/monitor-v2/dashboard`）存在严重的卡顿问题：
- **现象**: 页面加载时卡顿5-10秒，用户体验极差
- **影响范围**: 仅监控总览页面，其他页面正常
- **用户反馈**: "整个页面会卡5秒钟，我32G运存 + I5 11500都这样！"

#### 性能诊断过程

##### 1. 初步诊断
添加性能监控代码，发现：
- ✅ **API响应时间**: 200-300ms（正常）
- ✅ **JavaScript执行**: 300-400ms（正常）
- ❌ **用户感知延迟**: 5-10秒（异常）

```javascript
// 监控代码输出
✅ Dashboard API加载完成: 287ms
✅ Dashboard 浏览器渲染完成: 339ms
⚠️ 长任务阻塞: 248ms - self
⚠️ 长任务阻塞: 92ms - self
```
...
| 组件 | 移除后效果 | 结论 |
|------|-----------|------|
| BatchStatusChart（饼图） | 还是卡 | ✅ 不是问题组件 |
| TaskTypeChart（柱状图） | 还是卡 | ✅ 不是问题组件 |
| RelationChart（链路图） | **完全不卡了！** | ❌ **问题组件** |

**关键测试数据对比**：
```
有RelationChart:  卡5-10秒（用户感知）
无RelationChart:  ✅ 路由切换完成: 384ms
                 ✅ Dashboard 浏览器渲染完成: 327ms
                 ✅ 一点都不卡了！
```

#### 根因分析

`RelationChart.vue` 组件的性能问题：
...
const relationChains = computed(() => {
  configs.value.forEach(config => {
    ...
    const configTaskLinks = links.filter(link => 
      ...
      tasks.value.find(t => t.name === link.target)  // O(M) 嵌套
    )
    
    configTaskLinks.forEach(link => {
      // O(K)
      const taskBatchLinks = links.filter(l =>  // O(L) 再次嵌套
        l.source === task.name && 
        batches.value.find(b => b.name === l.target)  // O(P) 四层嵌套！
      )
    })
  })
})

// 时间复杂度: O(N * M * K * L * P) ≈ O(n^5) 级别！
```

##### 2. **DOM渲染量巨大**
原设计使用 `el-collapse` 嵌套结构：
```vue
<el-collapse>  <!-- 配置层 -->
  <el-collapse-item v-for="config">  <!-- 5个配置 -->
    <el-collapse>  <!-- 任务层 -->
      <el-collapse-item v-for="task">  <!-- 每个配置3-5个任务 -->
        <div v-for="batch">  <!-- 每个任务2-5个批次 -->
          <!-- 总计可能渲染: 5 * 5 * 5 = 125+ 个collapse组件 -->
```

**问题**：
- 每个 `el-collapse` 都有动画和交互逻辑
- 大量嵌套的 collapse 导致 Layout/Paint 开销极大
- 即使数据量不大（5个配置），也会卡死

##### 3. **Computed重复计算**
每次数据变化都触发完整的链路关系重新计算：
- `configs` computed
- `tasks` computed  
- `batches` computed
- `relationChains` computed（最复杂）
...
- ❌ 放弃复杂的树形/折叠结构
- ✅ 使用简单的 `el-table` 展示
- ✅ 优化算法：将 O(n^5) 降低到 O(n^2)
- ✅ 减少DOM渲染：从125+个组件降到10+个

**新组件代码**：`RelationTable.vue`

```vue
<el-table :data="tableData">
  <el-table-column prop="configName" label="配置名称" />
  <el-table-column prop="chainType" label="链类型" />
  <el-table-column prop="taskCount" label="关联任务" />
  <el-table-column prop="batchCount" label="产生批次" />
  <el-table-column label="任务详情">
    <template #default="{ row }">
      <el-tag v-for="task in row.tasks">
        {{ task.name }} ({{ task.batchCount }}批次)
      </el-tag>
    </template>
  </el-table-column>
</el-table>
  ...
  configs.value.forEach(config => {
    ...
    links.forEach(link => {
      if (link.source === config.name) {
        const task = tasksMap.get(link.target)  // O(1) 使用Map
        if (task) {
          row.taskCount++
          row.tasks.push(...)
        }
...
// 新的时间复杂度: O(N * M) ≈ O(n^2) - 降低了3个数量级！
```

#### 性能对比

| 指标 | 优化前（RelationChart） | 优化后（RelationTable） | 提升 |
|------|------------------------|------------------------|------|
| **用户感知延迟** | 5-10秒 | 0.3秒 | **30倍** |
| **页面渲染时间** | 未测量（太慢） | 327ms | - |
| **DOM节点数量** | ~125+ collapse | ~10 table rows | **12倍减少** |
| **算法复杂度** | O(n^5) | O(n^2) | **1000倍** |
| **Computed触发** | 5个复杂计算 | 2个简单计算 | **2.5倍减少** |

#### 实施结果

✅ **问题完全解决**：
- 从首页切换到监控总览：**384ms**（< 0.5秒）
- 页面完全渲染完成：**327ms**
- 用户反馈："一点都不卡了！"
...
##### 2. **常见性能陷阱**
- ❌ **嵌套循环地狱**：computed里多层filter/find嵌套
- ❌ **过度设计UI**：不必要的折叠、动画、嵌套组件
- ❌ **忽视DOM数量**：100+个组件vs 10个组件差距巨大

##### 3. **Vue性能监控技巧**
```javascript
// 1. 使用requestAnimationFrame监控真实渲染
requestAnimationFrame(() => {
  console.log('真实渲染完成')
})

// 2. 使用PerformanceObserver监控长任务
...
- [ ] 如果数据量增长到100+配置，考虑添加分页
- [ ] 考虑添加"展开详情"功能，按需加载批次信息
- [ ] 评估是否需要虚拟滚动（当前数据量不需要）
...
在代码Review和实际测试中发现5个关键问题，其中4个需要立即修复：
...
**问题描述**：
- 原实现：先删除旧批次 → 创建新批次 → 更新epoch
- 风险：若Consumer为空或分配失败，任务进入"无批次"状态
- 违背：零停机迁移/切换的核心设计目标
...
3. ✅ **再删除旧epoch批次**（只删除 epoch < newEpoch 的数据）
4. ✅ **失败回滚**：分配失败时保留旧批次，不更新epoch

**影响文件**：
```
ruoyi-crypto/src/main/java/com/ruoyi/crypto/service/impl/SmartBatchServiceImpl.java
  - 调整批次分配逻辑顺序
  - 添加分配失败回滚机制

ruoyi-crypto/src/main/java/com/ruoyi/crypto/mapper/MonitorBatchMapper.java
  - 更新删除方法签名，支持epoch参数
  - deleteBatchItemsByTaskId(Long taskId, Integer beforeEpoch)
  - deleteBatchesByTaskId(Long taskId, Integer beforeEpoch)

ruoyi-crypto/src/main/resources/mapper/MonitorBatchMapper.xml
  - 修改删除SQL，只清理旧epoch批次
  - WHERE task_id = #{param1} AND epoch < #{param2}
```

**效果验证**：
- ✅ Consumer始终能读取到批次（新或旧）
- ✅ 分配失败不影响现有批次
- ✅ 支持重复执行同步（新旧批次可短暂共存）
...
**问题描述**：
- 原实现：只要 `hasTwitter != null` 就强制 `requireTwitter=true`
- 前端提供：不限/必须有/必须无 三个选项
- 问题：用户选择"无Twitter"反而筛选"有Twitter"的项目

**修复方案**：
对齐sol监控的Twitter筛选逻辑，支持4种精确模式：

| 值 | 说明 | SQL逻辑 |
|---|------|---------|
| `null` | 不限 | 不添加筛选条件 |
| `"profile"` | 推特主页 | `twitter_url NOT LIKE '%/status/%' AND NOT LIKE '%/communities/%'` |
| `"tweet"` | 推文 | `twitter_url LIKE '%/status/%'` |
| `"community"` | 社区 | `twitter_url LIKE '%/communities/%'` |
| `"none"` | 无推特 | `twitter_url IS NULL OR twitter_url = ''` |

**影响文件**：
```
ruoyi-crypto/src/main/java/com/ruoyi/crypto/service/impl/SmartBatchServiceImpl.java
  - 修改：conditions.put("hasTwitter", task.getHasTwitter())
  - 删除：强制 requireTwitter=true 的逻辑

ruoyi-crypto/src/main/resources/mapper/crypto/TokenLaunchHistoryMapper.xml
  - 新增：<choose> 分支实现4种筛选SQL
  - 对齐：与sol监控的Twitter筛选逻辑完全一致
```

**效果验证**：
- ✅ "推特主页"：只返回主页链接（不含/status/等）
- ✅ "推文"：只返回推文链接
- ✅ "社区"：只返回社区链接
- ✅ "无推特"：只返回twitter_url为空的记录
...
**问题描述**：
- 定义了 `@Value("${monitor.batch.max-targets:10000}")`
- 但代码写死 `conditions.put("maxTargets", 20000)`
- 导致配置参数失效

**修复方案**：
```java
// 修复前
conditions.put("maxTargets", 20000);  // ❌ 硬编码

// 修复后
conditions.put("maxTargets", this.maxTargets);  // ✅ 使用配置
```

**影响文件**：
```
ruoyi-crypto/src/main/java/com/ruoyi/crypto/service/impl/SmartBatchServiceImpl.java
  - 第370行：使用 this.maxTargets 代替硬编码值
```

**效果验证**：
- ✅ 运维可通过 `application.yml` 调整上限
- ✅ 默认值：10000（可配置）
- ✅ 无需修改代码重新部署
...
**问题描述**：
- 为每个CA遍历 `allTargets` 列表查找对应的 `MonitorTaskTarget` 对象
- 时间复杂度：O(n²)
- 性能影响：1万条数据需要1亿次比较（10000 × 10000）

**修复方案**：
使用 `Map<String, MonitorTaskTarget>` 直接查找：

```java
// 修复前：O(n²) 性能问题
Map<String, Long> caToTargetIdMap = new HashMap<>();  // 只建ID映射
for (String ca : batchCAs) {
    Long targetId = caToTargetIdMap.get(ca);
    MonitorTaskTarget target = null;
    for (MonitorTaskTarget t : allTargets) {  // ❌ 遍历整个列表
...
Map<String, MonitorTaskTarget> caToTargetMap = new HashMap<>();  // ⭐ 新增
for (MonitorTaskTarget target : allTargets) {
    caToTargetIdMap.put(target.getCa(), target.getId());
    caToTargetMap.put(target.getCa(), target);  // ⭐ 建立完整对象映射
}

for (String ca : batchCAs) {
    MonitorTaskTarget target = caToTargetMap.get(ca);  // ✅ O(1) 查找
}
```
...
|--------|--------|--------|------|
| 1000条 | ~10秒 | <1秒 | 10倍 |
| 6000条 | 5-10分钟 | 3-5秒 | 100倍+ |
| 10000条 | 未测量（太慢） | <10秒 | 估计200倍+ |
...
- ✅ **架构正确**：实际是 **1对多** 关系
  - 一个任务 → 一个配置（1:1）
  - 一个配置 → 多个任务（1:N）
- ✅ **设计合理**：一个配置包含完整的监控规则（多个事件、条件），足够一个任务使用
- ✅ **无需修改**：当前实现符合业务需求
...
| 问题 | 优先级 | 状态 | 影响 |
|------|--------|------|------|
| 零停机失败 | 🔴 P0 | ✅ 已修复 | 确保Consumer无中断 |
| Twitter筛选 | 🟡 P1 | ✅ 已修复 | 用户可精确筛选 |
| maxTargets硬编码 | 🟡 P1 | ✅ 已修复 | 配置灵活化 |
| O(n²)性能 | 🟠 P2 | ✅ 已修复 | 性能提升100倍+ |
| 任务配置关系 | - | ❌ 非问题 | 架构正确 |

**修改文件统计**：
- Java文件：2个（SmartBatchServiceImpl.java, MonitorBatchMapper.java）
- XML文件：2个（MonitorBatchMapper.xml, TokenLaunchHistoryMapper.xml）
- 总代码行数：~50行修改

**测试建议**：
1. ✅ 零停机测试：多次执行智能同步，验证Consumer无中断
2. ✅ Twitter筛选测试：验证4种模式是否正确筛选
3. ✅ 配置测试：修改yml中的maxTargets，验证生效
4. ✅ 性能测试：创建10000条目标的任务，验证<10秒完成

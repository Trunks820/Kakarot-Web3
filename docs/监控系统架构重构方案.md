...
- **任务层（Task）**：定义一次具体监控任务（类型/链别/调度策略），绑定一个或多个配置
...
#### ✅ **配置与任务多对多**
- 一个任务可以组合多个配置（如：价格监控 + 持仓监控 + 成交量监控）
...
#### 3. **monitor_task_config** (任务配置关联表) - 多对多映射
...
| 任务配置关联表 | M:N关联 | `monitor_task_config_v2` | ✅ |
